...
class SHHSampleGetting:

    def __init__(self, x=None, y=None, get_type="unique"):
        self.x = None
        self.y = None
        self.get_select_list = None
        self.get_type = get_type
        self.getting_cn_dict = {}
        self.origin_cn_dict = {}
    ...
    def init(self, x, y):
        ...
        if self.y is not None:
            self.get_select_list = np.ones(len(self.y), dtype=bool)
            self.calGCN()

    def calGCN(self):
        categorys, n = np.unique(self.y[self.get_select_list], return_counts=True)
        for i in range(len(categorys)):
            self.origin_cn_dict[categorys[i]] = n[i]

    def addCategoryNumber(self, *cns):
        self.calGCN()
        for i in range(len(cns) - 1):
            category = cns[i]
            n = cns[i + 1]
            self.addCN(category, n)

    def addCategorysNumber(self, *cns):
        self.calGCN()
        category = cns[:-1]
        n = cns[-1]
        self.addCN(category, n)

    def addCN(self, category, n):
        self.getting_cn_dict[category] = n
        # if category not in self.origin_cn_dict:
        #     if category is None:
        #         self.getting_cn_dict[category] = n
        #     else:
        #         warnings.warn("category \"{0}\" not in categorys.".format(category))
        # else:
        #     self.getting_cn_dict[category] = n
        #     if category is not None:
        #         if n is not None:
        #             if n != self.origin_cn_dict[category]:
        #                 warnings.warn("number of category of \"{0}\" {1} not eq {2}".format(
        #                     category, self.origin_cn_dict[category], n))

    def getDataFrame(self):
        self.calGCN()
        select_list_index_list = self.randomSelectList()
        df = self.x.loc[select_list_index_list]
        y = self.y[select_list_index_list]
        return df, y

    def randomSelectList(self):
        select_list_index_list = []
        for category in self.getting_cn_dict:
            select_list_index_list += self.randomSelect(category, self.getting_cn_dict[category])
        return select_list_index_list

    def randomSelect(self, category, n):
        select_list = np.ones(len(self.y), dtype=bool)
        select_list[self.get_select_list] = True
        if category is None:
            select_list_index_ret = self.randomSelectNumber(n, select_list)
            return select_list_index_ret

        select_list_index_ret = []
        if not isinstance(category, tuple):
            category = [category]
        for i in range(len(category)):
            cate = category[i]
            select_list_ = np.logical_and(select_list, self.y == cate)
            select_list_index = self.randomSelectNumber(n, select_list_)
            select_list_index_ret += select_list_index

        return select_list_index_ret

    def randomSelectNumber(self, n, select_list_):
        select_list_index = self._boolIndex(select_list_)
        n_temp = len(select_list_index)
        if n_temp == 0:
            return []
        random.shuffle(select_list_index)
        if n > n_temp:
            n = n_temp
        select_list_index = select_list_index[:n]
        self.get_select_list[select_list_index] = False
        return select_list_index

    def _boolIndex(self, select_list, obj_bool=True):
        ret_list = []
        for i in range(len(select_list)):
            if bool(select_list[i]) == obj_bool:
                ret_list.append(i)
        return ret_list

    def printOriginCategoryNumber(self):
        to_list = []
        for k in self.origin_cn_dict:
            print("{0}:{1}, ".format(k, self.origin_cn_dict[k]), end="")
            # to_list.append([k, self.origin_cn_dict[k]])
        print()
        # printListTable(to_list)

    def tiaoSHHierarchicalModel_reCategorys(self, re_categorys, re_categorys_number):
        self.getting_cn_dict = {}
        if (re_categorys_number is None) or (re_categorys_number == (None,)):
            re_categorys_number = [None for i in range(len(re_categorys))]
        categorys = ()
        for i, category in enumerate(re_categorys):
            n = re_categorys_number[i]
            if category is None:
                category = []
                for o_cate in self.origin_cn_dict.keys():
                    if o_cate not in categorys:
                        category.append(o_cate)
                category = tuple(category)
                if n is None:
                    n = self.getOriginNumber(*category)
                self.addCategoryNumber(category, n)
                continue
            category = tuple(category)
            categorys += category
            if n is None:
    ...
    def getOriginNumber(self, *categorys):
        n = 0
        for category in categorys:
            n += self.origin_cn_dict[category]
        return n


class SHHSampleSelect(SRTSampleSelect):
    ...
    def __init__(self, x: pd.DataFrame = None, y=None, sampling_type="no_back"):
        ...
        """
        super(SHHSampleSelect, self).__init__(x=x, y=y, sampling_type=sampling_type)
...
class SHHModel(SRTModelInit):
    ...
    def addReCategoryNumber(self, *numbers):
        self.re_categorys_number = numbers
...
def trainModel(train_args, model: SHHModel, x_test, x_train, y_test, y_train):
...
class ShadowHierarchicalModel(SHHModel):
    ...
    def train(self, x_train, y_train, x_test, y_test, *args, **kwargs):
        ...
        ssh_sg = SHHSampleGetting(self.x_train, self.y_train, )
    ...
    def trainSampleNumber(self, x_train, y_train, x_test, y_test):
        self.veg_mod = self.models["VEG"]
        self.is_low_mod = self.models["LOW_NO"]
        self.is_soil_mod = self.models["IS_SOIL"]
        self.ws_mod = self.models["WS"]
        ...
        self.x_train, self.y_train, self.x_test, self.y_test = x_train, y_train, x_test, y_test
        self.field_names = list(self.x_train.keys())
        shh_ss = SHHSampleSelect(self.x_train, self.y_train)

        shh_sg = SHHSampleGetting(self.x_train, self.y_train, )
        shh_sg.printOriginCategoryNumber()
        shh_sg.tiaoSHHierarchicalModel_reCategorys(self.veg_mod.re_categorys, self.veg_mod.re_categorys_number)
        x, y = shh_sg.getDataFrame()
        print(self.veg_mod.name, end=": ")
        shh_sg.printOriginCategoryNumber()
        shh_sg.tiaoSHHierarchicalModel_reCategorys(self.is_low_mod.re_categorys, self.is_low_mod.re_categorys_number)
        x, y = shh_sg.getDataFrame()
        print(self.is_low_mod.name, end=": ")
        shh_sg.printOriginCategoryNumber()
        shh_sg.tiaoSHHierarchicalModel_reCategorys(self.is_soil_mod.re_categorys, self.is_soil_mod.re_categorys_number)
        x, y = shh_sg.getDataFrame()
        print(self.is_soil_mod.name, end=": ")
        shh_sg.printOriginCategoryNumber()
        shh_sg.tiaoSHHierarchicalModel_reCategorys(self.ws_mod.re_categorys, self.ws_mod.re_categorys_number)
        x, y = shh_sg.getDataFrame()
        print(self.ws_mod.name, end=": ")
        shh_sg.printOriginCategoryNumber()
    ...
    def modelReCategoryNumber(self, name, *numbers):
        self.models[name].addReCategoryNumber(*numbers)
...
class ShadowHierarchicalTrainImdcOne(ShadowMain):
    ...
    def fit(self, is_sh_to_no=False, is_test_sample_number=False):
        ...
        if is_test_sample_number:
            self.model.trainSampleNumber(x_train, y_train, x_test, y_test)
            return
...
def run(shti_class):
    ...
    model.modelReCategoryNumber("VEG", None)
    ...
    model.modelReCategoryNumber("LOW_NO", None)
    ...
    model.modelReCategoryNumber("IS_SOIL", None)
    ...
    model.modelReCategoryNumber("WS", None)
    ...
    shtio_bj.fit(is_test_sample_number=True)
    shtio_bj.classify()
