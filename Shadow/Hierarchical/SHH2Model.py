...
from SRTCodes.Utils import datasCaiFen, changext, Jdt, saveJson, readJson, DirFileName, FN
...
class _MLSamples(_Samples):
    ...
    def deal(self):
        self.x_train = self.data_scale.fits(pd.DataFrame(self.x_train))
...
class SamplesData:
    ...
    def addDLCSV(self, csv_fn, read_size, data_get_keys, x_deal=None):
        ...
        if not os.path.isfile(csv_fn_spl):
            samplingCSVData(csv_fn, csv_fn_spl, npy_fn, names_fn, read_size[0], read_size[1])
    ...
    def ml(self, x_keys, map_dict, data_scale=_DataScale(), train_filters=None, test_filter=None):
...
class _GDALImdc(GDALImdc):
    ...
    def __init__(self, *raster_fns, data_scale: _DataScale = None):
...
def _imdc1(fn, mod, x_keys, raster_fns, data_scale, color_table, to_imdc_fn):
    if to_imdc_fn is None:
        to_imdc_fn = changext(fn, "_imdc.tif")
    gimdc = _GDALImdc(raster_fns, data_scale=data_scale)
    gimdc.imdc1(mod, to_imdc_fn, x_keys, color_table=color_table)
    return to_imdc_fn


def _imdc2(fn, mod, raster_fns, data_scale, win_size, to_imdc_fn, fit_names, color_table, n=1000):
    if to_imdc_fn is None:
        to_imdc_fn = changext(fn, "_imdc.tif")
    gimdc = _GDALImdc(raster_fns, data_scale=data_scale)
...
class _ModelInit:
    ...
    def imdc(self, *args, **kwargs):
        ...
        if raster_fn is None:
            return None
        to_imdc_fn = _imdc1(self.filename, self, self.x_keys, raster_fn, self.data_scale, self.color_table, to_imdc_fn)
        return to_imdc_fn
...
class MLModel(_ModelInit):
    ...
    >>> ml_mod.data_scale = _DataScale().readJson("json_fn")
    ...
    def imdc(self, raster_fns, to_imdc_fn=None, *args, **kwargs):
        to_imdc_fn = _imdc1(self.filename, self, self.x_keys, raster_fns, self.data_scale, self.color_table, to_imdc_fn)
        return to_imdc_fn
