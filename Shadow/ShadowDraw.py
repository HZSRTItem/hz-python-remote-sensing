...
from SRTCodes.GDALRasterIO import samplingGDALRastersToCSV, GDALRasterChannel, readGEORaster, getGDALRasterNames
from SRTCodes.NumpyUtils import filterEq, reHist
...
def tempFuncs():
    ...
    def func1():
        ...
        x_draw_feat, y_draw_feat = "NDVI", "NDWI"
...
def method_name1():
    ...
    def func1(raster_fn):
        ...
        k_list = [field_names.index(k) for k in
                  ["AS_VV", "AS_VH", "AS_C11", "AS_C22", "AS_Lambda1",
                   "AS_Lambda2", "AS_SPAN", "AS_Epsilon", "DE_VV", "DE_VH", "DE_C11",
                   "DE_C22",
    ...
    #     sdhs.plot(field_name, bins=1000)
    #     hist_dict["{0}_bins".format(field_name)] = sdhs.bin_edges[:-1].tolist()
    #     hist_dict["{0}_hist".format(field_name)] = sdhs.hist.tolist()
    #     image_fn = os.path.join(r"F:\ProjectSet\Shadow\Analysis\4\ChengDu", "{0:03d}_{1}_ChengDu.jpg".format(i + 1, field_name))
    #     print(image_fn)
    #     plt.legend()
    #     plt.savefig(image_fn, format="jpeg")
    #     plt.close()
    # savecsv(r"F:\ProjectSet\Shadow\Analysis\4\ChengDu\cd_hist.csv", hist_dict)
