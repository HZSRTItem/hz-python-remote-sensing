...
JPZ2M_MDW_DFN = DirFileName(r"K:\zhongdianyanfa\jpz_5\good5m2017-2018\madewang")
...
class ZY2MMdw_Network(nn.Module):
...
class ZY2MMdw_Network_res(nn.Module):
...
class ZY2MMdw_Network2(nn.Module):
...
class ZY2MMdw_Network3(nn.Module):
...
class ZY2MMdw_DataSet(SRTDataset, Dataset):
...
class ZY2MMdw_PytorchTraining(PytorchCategoryTraining):
...
class ZY2MMdw_GDALRasterPrediction(GDALRasterPrediction):

    def __init__(self, geo_fn):
        super(ZY2MMdw_GDALRasterPrediction, self).__init__(geo_fn)
        self.device = "cuda:0"
...
class ZY2MMdw_Main:
    ...
    def __init__(self):
        self.this_dirname = self.mkdir(r"H:\JPZ\ZY2MMdw")
        ...
        self.csv_fn = JPZ2M_MDW_DFN.fn(r"Samples\zy2m_mdw_train6.csv")
        self.npy_fn = JPZ2M_MDW_DFN.fn(r"Samples\zy2m_mdw_train6_sum.npy")
        self.test_ds = None
        self.train_ds = None
        self.win_size = 5
        self.mod = ZY2MMdw_Network_res(in_channel=4, win_size=self.win_size, n_category=1)
        self.loss = ZY2MMdw_Loss()
        self.geo_raster = JPZ2M_MDW_DFN.fn(r"GF1_mdw2.tif")
    ...
    def train(self):
        ...
        self.train_ds = ZY2MMdw_DataSet(is_test=False, spl_fn=self.csv_fn, train_d_fn=self.npy_fn, cname="CATEGORY")
        self.test_ds = ZY2MMdw_DataSet(is_test=True, spl_fn=self.csv_fn, train_d_fn=self.npy_fn, cname="CATEGORY")

        pytorch_training = ZY2MMdw_PytorchTraining(
        ...
        for f in os.listdir(raster_dirname):
            if os.path.splitext(f)[1] == '.tif':
                ...
                if os.path.isfile(imdc_fn):
                    ...
                    continue
                grp = ZY2MMdw_GDALRasterPrediction(geo_raster)
    ...
    def imdcOne(self, mod_fn=None):
        ...
        grp = ZY2MMdw_GDALRasterPrediction(self.geo_raster)
    ...
    def warp(self):
        ...
        zy5m_grw.warpImage(
            JPZ2M_MDW_DFN.fn(r"GF1_PMS2_E103.2_N13.1_20180325_L1A0003082609\spl1.csv"),
            JPZ2M_MDW_DFN.fn(r"GF1_PMS2_E103.2_N13.1_20180325_L1A0003082609\GF1_mdw.dat"),
            JPZ2M_MDW_DFN.fn(r"GF1_PMS2_E103.2_N13.1_20180325_L1A0003082609\GF1_mdw2.dat"),
        )
...
if __name__ == "__main__":
    """
    F:\PyCodes\ZY5M\ZY2MMdwC.py
    run import sys; sys.path.append(r'F:\PyCodes'); from Y5M.ShadowData import ZY2MMdwC; zy2m_admj = ZY2MMdw_Main();zy2m_admj.imdcOne()
    python -c "import sys; sys.path.append(r'F:\PyCodes'); from ZY5M.ZY2MMdwC import imdcOne; imdcOne(r'H:\JPZ\ZY2MMdw\Mods\20231119H204151\mmodel_44.pth')"
    python -c "import sys; sys.path.append(r'F:\PyCodes'); from ZY5M.ZY2MMdwC import main; main()"
    """
    # main()
    imdcOne(r"H:\JPZ\ZY2MMdw\Mods\20231120H224012\model_epoch_96.pth")
