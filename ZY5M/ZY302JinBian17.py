...
r"""----------------------------------------------------------------------------
@File    : ZY302JinBian17.py
@Time    : 2023/9/21 21:21
@Author  : Zheng Han 
@Contact : hzsongrentou1580@gmail.com
@License : (C)Copyright 2023, ZhengHan. All rights reserved.
@Desc    : PyCodes of ZY302JinBian17
-----------------------------------------------------------------------------"""
import os.path
...
ZY3_JINBIAN_DFN = DirFileName(r"K:\zhongdianyanfa\jpz_5\good5m2017-2018\ZY302_PMS_E105.0_N11.6_20180802_L1A0000356944")


class ZY302JB17_Network(nn.Module):
...
class ZY302JB17_Loss(nn.Module):
...
class ZY302JB17_DataSet(SRTDataset, Dataset):

    def __init__(self, is_test=False):
        super().__init__()

        self.csv_fn = ZY3_JINBIAN_DFN.fn(r"samples\jinbian2017_spl3_3.csv")
        self.npy_fn = ZY3_JINBIAN_DFN.fn(r"samples\jinbian2017_spl3_3\sum.npy")

        self.spl_fn = ZY3_JINBIAN_DFN.fn(r"samples\jinbian2017_spl3_3.csv")
        self.train_d_fn = ZY3_JINBIAN_DFN.fn(r"samples\jinbian2017_spl3_3\sum.npy")
    ...
    def get(self, index):
        ...
        x = np.clip(x, 0, 500) / 500.0
        y = self.category_list[index]
        return x, y


class ZY302JB17_PytorchTraining(PytorchCategoryTraining):
...
class ZY302JB17_GDALRasterPrediction(GDALRasterPrediction):

    def __init__(self, geo_fn):
        super(ZY302JB17_GDALRasterPrediction, self).__init__(geo_fn)
        self.device = "cuda:0"
...
class ZY302JB17_Main:

    def __init__(self):
        self.this_dirname = self.mkdir(r"H:\JPZ\ZY302JB17")
        ...
        self.csv_fn = ZY3_JINBIAN_DFN.fn(r"samples\jinbian2017_spl3_3.csv")
        self.npy_fn = ZY3_JINBIAN_DFN.fn(r"samples\jinbian2017_spl3_3\sum.npy")
        self.train_ds = ZY302JB17_DataSet(is_test=False)
        self.test_ds = ZY302JB17_DataSet(is_test=True)
        self.win_size = 9
        self.mod = ZY302JB17_Network(4, 9, 1)
        self.loss = ZY302JB17_Loss()
        self.geo_raster = ZY3_JINBIAN_DFN.fn("ZY302_JinBin17_2.tif")
    ...
    def train(self):
        pytorch_training = ZY302JB17_PytorchTraining(
    ...
    def imdc(self):
        grp = ZY302JB17_GDALRasterPrediction(self.geo_raster)
        mod_dirname = "20230921H213235"
        imdc_fn = os.path.join(self.model_dir, mod_dirname, "imdc1.tif")
        mod_fn = os.path.join(self.model_dir, mod_dirname, "model_462.pth")
...
def method_name():
    # 使用CSV文件在影像中提取样本的数据
    spl_fn = ZY3_JINBIAN_DFN.fn(r"samples\jinbian2017_spl3_3.csv")
    raster_fn = ZY3_JINBIAN_DFN.fn("ZY302_JinBin17_2.tif")
    train_d_fn = ZY3_JINBIAN_DFN.fn(r"samples\jinbian2017_spl3_3\sum.npy")
    spl_size = [9, 9]
