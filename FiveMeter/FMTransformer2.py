...
from SRTCodes.Utils import readJson, writeCSVLine, readcsv, Jdt, savecsv, saveJson
...
class FMDataSet(Dataset):
    ...
    def __getitem__(self, index):
        x = self.data[index]
        x = x / 1600
        y = self.json_data[index]["CATEGORY_CODE"]
        return x, y
...
def samplingCenter(raster_fn, csv_fn, to_fn, spl_size, channel_list=None,
...
class FMTMain:
    ...
    def trainOCC(self):
        ...
        print("Export Test Data")
        test_dataset = FMDataSet(
            np_filename=r"F:\ProjectSet\FiveMeter\TransformerSample\1\trans_spl1_1_spl.npy",
            json_filename=r"F:\ProjectSet\FiveMeter\TransformerSample\1\trans_spl1_1_spl.csv.json",
        )
        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
    ...
    def sampling(self):
        to_csv_fn = samplingCenter(
            raster_fn=r"G:\GraduationProject\QingDaoImages\RGB_VV_VH\qdgrid_im_6_tif.tif",
            csv_fn=r"F:\ProjectSet\FiveMeter\TransformerSample\1\trans_spl1_1.csv",
            to_fn=r"F:\ProjectSet\FiveMeter\TransformerSample\1\trans_spl1_1_spl",
            spl_size=[12, 12]
        )
        df = pd.read_csv(to_csv_fn)
        data = [df.loc[i].to_dict() for i in range(len(df))]
        saveJson(data, to_csv_fn + ".json")
