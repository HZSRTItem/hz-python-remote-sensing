...
WSL_ROOTFS_DIR = r"D:\WSL2\Ubuntu_2204.1.7.0_x64"
...
def get_winsys_envvar(k):
    ...
    return subprocess.check_output(
        r'powershell.exe -Command "echo $%s"' % k,
        shell=True,
...
def wlfConvert(path, path_format="unix", abs_path_option=True, doubledash_path_option=False, win_path_type=None):
    convert_path = ""
    path = [path]
    p = path
    if win_path_type:
        ...
        if not is_abs and not abs_path_option:
            return convert_relative_path(p)
        if not is_abs:
            p = os.path.abspath(p)

    if path_format == 'unix':
        return convert_to_wsl_path(p)
    elif path_format == 'windows':
        return convert_to_win_path(doubledash_path_option, p)
...
def W2LF(path, ):
    return wlfConvert(path)


def main():
    print(W2LF(r"WSL2/filena"))
    print(os.path.abspath(os.path.join("WSL2", "dsafsd", "dsfsd")))
    pass
